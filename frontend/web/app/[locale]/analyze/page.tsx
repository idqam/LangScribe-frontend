      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">
            NLP Analysis Playground
          </h1>
          <p className="text-slate-600">
            Test the browser-based CEFR estimation pipeline.
          </p>
        </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-slate-500">
                    Language: {result.lang === "en" ? "English" : "Spanish"}
                  </p>
                </div>
                ...
                <div className="border-t border-slate-200 pt-6">
                  ...
                  <div className="space-y-3">
                    {result.grammarErrors.map((error: any, index: number) => (
                      <div
                        key={index}
                        className="flex items-start gap-3 p-3 bg-red-50 text-red-700 rounded-lg text-sm"
                      >
                        <span className="font-medium shrink-0">
                          {error.rule}:
                        </span>
                        <span>
                          {error.message} (Match: "{error.match}")
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {result.features.vocab.typeTokenRatio.toFixed(2)} (TTR)
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Unknown Words</span>
                    <span className="font-medium">
                      {(result.features.vocab.pctUnknownWords * 100).toFixed(1)}
                      %
                    </span>
                  </div>
                  <div className="pt-2 border-t border-slate-100">
                    ...
                    {Object.entries(result.features.vocab.cefrProportions).map(
                      ([level, prop]: any) => (
                        <div
                          key={level}
                        ...
                        >
                          ...
                          <span className="w-8 text-xs text-right text-slate-600">
                            {(prop * 100).toFixed(0)}%
                          </span>
                        </div>
                      )
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {result.features.syntax.avgSentenceLength.toFixed(1)}{" "}
                      words
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {result.features.syntax.clauseEstimate}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Verb Tenses</span>
                    <span className="font-medium">
                      {result.features.morph.verbTenseVariety} types
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Discourse Markers</span>
                    <span className="font-medium">
                      {result.features.cohesion.discourseMarkerCount}
                    </span>
                  </div>
                </div>
              </div>
