...
const QuickStats = ({ entries }: { entries: any[] }) => {
  ...
  const totalWords = entries.reduce(
    (sum, entry) => sum + (entry.analysis?.tokens?.length || 0),
    0
  );
  ...
  const avgQuality =
    totalEntries > 0
      ? Math.round(
          (entries.reduce(
            (sum, entry) => sum + (entry.analysis?.subscores?.grammar || 0),
            0
          ) /
            totalEntries) *
            100
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="grid grid-cols-7 gap-2 w-full max-w-sm">
          ...
          {activity.map((val, i) => (
            <div
              key={i}
              className={`aspect-square rounded-md ${getColor(
                val
...
const VocabularyChart = ({ entries }: { entries: any[] }) => {
  ...
  const displayWords =
    recentWords.length > 0
      ? [...new Set(recentWords)].slice(0, 5)
      : ["learn", "write", "speak", "study", "grow"];
...
const LevelProgress = ({ entries }: { entries: any[] }) => {
  ...
  const currentLevel =
    entries.length > 0 ? entries[0].analysis.cefrLevel : "A1";

  const levels = [
    {
      level: "A1",
      name: "Beginner",
      progress: currentLevel >= "A1" ? 100 : 0,
      color: "bg-emerald-500",
    },
    {
      level: "A2",
      name: "Elementary",
      progress: currentLevel >= "A2" ? 100 : 0,
      color: "bg-emerald-500",
    },
    {
      level: "B1",
    ...
    },
    {
      level: "C1",
      name: "Advanced",
      progress: currentLevel >= "C1" ? 100 : 0,
      color: "bg-slate-300",
    },
    {
      level: "C2",
      name: "Proficiency",
      progress: currentLevel >= "C2" ? 100 : 0,
      color: "bg-slate-300",
    },
  ];
