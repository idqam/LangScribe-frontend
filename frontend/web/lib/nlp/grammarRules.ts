...
export const checkGrammar = (tokens: Token[], text: string, lang: 'en' | 'es'): GrammarError[] => {
    ...
    if (lang === 'en') {
        ...
        for (let i = 0; i < tokens.length - 1; i++) {
            if (tokens[i].normal === 'to' && tokens[i + 1].tags.includes('PastTense')) {
                errors.push({
        ...
        for (let i = 0; i < tokens.length - 1; i++) {
            if (['he', 'she', 'it'].includes(tokens[i].normal) &&
                tokens[i + 1].tags.includes('PresentTense') &&
                !tokens[i + 1].text.endsWith('s') &&
                !['is', 'has', 'was', 'does', 'can', 'will', 'should', 'could', 'would', 'may', 'might', 'must'].includes(tokens[i + 1].normal)) {
        ...
        for (let i = 0; i < tokens.length - 1; i++) {
            if (['he', 'she', 'it'].includes(tokens[i].normal) && tokens[i + 1].normal === 'are') {
                errors.push({
        ...
        for (let i = 0; i < tokens.length - 1; i++) {
            ...
            if (t1.tags.includes('Determiner') && t2.tags.includes('Noun')) {
                const isFem1 = t1.normal.endsWith('a') || t1.normal === 'la' || t1.normal === 'las' || t1.normal === 'una' || t1.normal === 'unas';
                const isMasc1 = t1.normal.endsWith('o') || t1.normal === 'el' || t1.normal === 'los' || t1.normal === 'un' || t1.normal === 'unos';

                const isFem2 = t2.normal.endsWith('a') || t2.normal.endsWith('as') || t2.normal.endsWith('ción') || t2.normal.endsWith('dad');
        ...
        if (['qué', 'quién', 'cómo', 'cuándo', 'dónde', 'por qué'].some(w => text.toLowerCase().includes(w)) && !text.includes('¿')) {
            errors.push({
